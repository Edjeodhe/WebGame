...
- 폼(코어) 시스템: 개미(근접) ↔ 장수풍뎅이(원거리) 2종 고정 스왑. 장수풍뎅이 폼에서는 기본 공격과 스킬이 모두 원거리 투사체로 나간다.
- 콤보 공격(스윙 궤적/무기 이동 연출 포함) + 코어 전용 스킬(코어마다 다른 이펙트)
...
- 스테이지 진행: 고정 3개 스테이지를 순서대로 진행하며, 스테이지가 오를수록 적 수/체력/피해량이 지수적으로 증가(`stageDifficultyMult`)
...
- 도트(픽셀 블록) 스프라이트: 이미지 파일 없이 `src/sprites.js`에서 작은 사각형을 격자에 합성해 실루엣을 표현(개미/장수풍뎅이 + 잡몹/군주)
...
| Space | 점프 |
| Shift | 대시(짧은 무적) |
| F | 기본 공격(장수풍뎅이 폼에서는 원거리) |
| Q | 코어 전용 스킬(장수풍뎅이 폼에서는 원거리) |
| W | 폼 전환(개미 ↔ 장수풍뎅이) |
| I | 인벤토리 열기/닫기(안식처에서만) |
| 1 / 2 / 3 | 레벨업 시 증강 선택 |

전투 키(F/Q/W)는 왼손이 이동(←→)에서 크게 벗어나지 않도록 근처에 모아뒀다.
